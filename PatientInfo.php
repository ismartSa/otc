...
if($id != null && $id > 0){
	...
	$result = mysql_query("SELECT * FROM Patient where PatientId = '$id'")or die ('Error updating database: '.mysql_error());
...
<h3><?= $row[1] ?></h3>
							 ...
							 <div class="col-xs-12 col-sm-4 col-md-4">
                                      <button id="input-patient" > history  <input type="hidden" value="<?= $row[0] ?>">
</button>
                                        </div>
    ...
    <script>
        ...
        $('#input-patient').click(function () {
            var code = $(this).find("input").val();
            if(code!=""){
          window.location.assign('PatientHistory.php?patient_id='+code);
        }
        });
